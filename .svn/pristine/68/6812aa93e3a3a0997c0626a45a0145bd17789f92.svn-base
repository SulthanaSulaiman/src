...
public class ChapterPlanDetails implements Serializable {
    ...
    public List getChapterPlanDet(ChapterPlanDetVO planRqst) {
        ...
        try {
            ...
            String chapterDetQry = " SELECT b.chapter_id,a.chapter_no,b.plan_id,b.milestone_id,c.milestone_act_name,DATE_FORMAT(b.end_date, '%Y-%m-%d'),b.planned_date,mo.milestone_order_number,b.plan_auto_id"
                    + " FROM chapter a,chapter_plan b,proj_milestone_act c,milestone_order mo,project_stage ps "
                    + " WHERE a.chapter_id=b.chapter_id AND b.milestone_id=mo.milestone_act_code AND b.chapter_id=? and mo.stage_code=? and mo.milestone_act_code=c.milestone_act_code and ps.stage_code=mo.stage_code "
                    + "ORDER BY b.order_no,mo.milestone_order_number,b.end_date ";

            chapterId = planRqst.getChapterId();
            stageCode = planRqst.getChapterStgCode();
            ...
            while (rs.next()) {
                ...
                response.setMileStoneOrderNo(rs.getString(8));
                response.setPlanAutoId(rs.getString(9));
    ...
    public List getChapterPlanCompletedDetails(ChapterPlanDetVO planRqst) {
        ...
        try {
            ...
            String chapterDetQry = " SELECT drof.chapter_id,drof.dropoff_to,drof.dropoff_by,drof.date "
                    + " FROM dropoff drof,chapter ch,proj_milestone_act pma "
                    + " WHERE drof.chapter_id=? AND ch.chapter_id=drof.chapter_id AND pma.milestone_act_code=drof.dropoff_to ";
            ...
            while (rs.next()) {
                response = new ChapterPlanDetVO();
                response.setChapterId(rs.getString(1));
                response.setMileStoneId(rs.getString(2));
                chapterPlan.add(response);
            }
    ...
    public List getChapterMilestoneCompletedDet(ChapterPlanDetVO planRqst) {
        ...
        try {
            ...
            String chapterDetQry = " select act.chapter_id,act.milestone_code,act.milestone_comp_flag "
                    + " from activity act,chapter ch "
                    + " where act.chapter_id=? and (act.end_time is not null or act.end_time!='0000-00-00 00:00:00') "
                    + " and ch.chapter_id=act.chapter_id group by act.milestone_code ";
