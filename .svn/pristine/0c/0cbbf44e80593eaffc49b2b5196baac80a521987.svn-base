 ...
 * @author thanuja
 */
public class EstimationPdfServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        lineItemDispCount = 0;
//        System.out.println("getEstimationNumber:" + getEstimationNumber);
//        System.out.println("GET - project_name:" + project_name);
        boolean exp = false;
        ...
        try {
            ...
            estDate = det.getEstimateDisplayDate();
                ...
                for(int c=0;c<line_item_no.size();c++)
                    {
                    lineItemDispChk=false;
                       emptyCheck=line_item_no.get(c).toString() !=null ? line_item_no.get(c).toString() : "";
           ...
           /* if(tempClient.contains("(") && tempClient.contains("(")) {
            ...
            if(billingLocation.equals("2")) {
                ...
                billingAddress = "12410 Milestone Center Drive, Suite 20" + "\n" + "Germantown, MD 20876" + "\n";
            }
            ...
            //Buyer address
//            PdfPCell addCell = new PdfPCell(new Paragraph(det.getBuyerAdd() + "\n" + det.getBuyerCity() + "" + det.getBuyerState() + "\nTelephone: " + det.getBuyerPhone() + "\nFax: " + det.getBuyerFax(), small));
//            addCell.setIndent(30);
//            addCell.setMinimumHeight(50f);
//            disableBorders(addCell);
//
//            Paragraph buyer = new Paragraph(det.getBuyerAdd() + "\n" + det.getBuyerCity() + "" + det.getBuyerState() + "\nTelephone: " + det.getBuyerPhone() + "\nFax: " + det.getBuyerFax(), small);
//            PdfPCell buyerAdd = new PdfPCell(buyer);
...
//            nestedTable.addCell(cell11);
            ...
            // Buyer Address if you want remove the command line below
          ...
          /*  if(!projInfo.getBuyerAddress().equals(null) && !projInfo.getBuyerAddress().equals("")) {
            ...
            Paragraph summ = new Paragraph("Estimation Summary", headerFont);
            summ.setIndentationLeft(5);
            document.add(summ);
            ...
            summTable.setWidthPercentage(50);
            ...
            summTable.addCell(emptyCell);

            //Get the table data
            PdfPCell cellv = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat.format(Double.parseDouble(summary.getEstValue().get(0).toString())), smallBold));
            cellv.setHorizontalAlignment(Element.ALIGN_CENTER);
            cellv.setBorderWidth(1f);
            summTable.addCell(cellv);
            summTable.addCell(emptyCell);
            //summTable.addCell(new Paragraph(summary.getEstStatus().get(0).toString(), smallBold));
            ...
            PdfPCell poNum = new PdfPCell(new Paragraph("Estimation Number: " + getEstimationDispNumber, headerFont));
            ...
            float[] propColWidth = {0.5f, 2.3f, 1f, 1.8f}; // Code 1
            ...
            disableBorders(authorVal);
            cell7.setMinimumHeight(8f);
            projProp.addCell(cell7);
            disableBorders(cell7);
            cell8.setMinimumHeight(8f);
            projProp.addCell(cell8);
            disableBorders(cell8);
            ...
            float[] lineItemsWidth1 = {7f, 7f, 25f, 9f, 10f, 10f, 10f, 10f};
            int width=0;
...
//            }
            ...
            for (int loop = 0; loop < item_id.size(); loop++) {
                ...
                if (loop + 1 == item_id.size()) {
                    ...
                    if(lineItemDispChk)
                    {
                    lastRow = new PdfPCell(new Paragraph(line_item_no.get(loop).toString(), small));
                    ...
                    lastRow = new PdfPCell(new Paragraph(""));
                    lastRow.setBorderWidth(1f);
                    lastRow.disableBorderSide(PdfPCell.TOP);
                    lineTable.addCell(lastRow);

                    if(item_desc.get(loop).toString().equals("")) {
                        lastRow = new PdfPCell(new Paragraph("        " + item_name.get(loop).toString(), small));
                    } else {
                        float[] noteWidths = {8f, 15f};
                        ...
                        c1 = new PdfPCell(new Paragraph(item_name.get(loop).toString(), small));
                        c1.setColspan(2);
                        c1.setBorder(Rectangle.NO_BORDER);
                        testTable.addCell(c1);
                        testTable.setWidths(noteWidths);

                        c1 = new PdfPCell(new Paragraph("    Note:", smallBoldNote));
                        c1.setHorizontalAlignment(Element.ALIGN_RIGHT);
                        c1.setBorder(Rectangle.NO_BORDER);
                        testTable.addCell(c1);
                        
                        c1 = new PdfPCell(new Paragraph(item_desc.get(loop).toString(), smallNote));
                        c1.setBorder(Rectangle.NO_BORDER);
                        testTable.addCell(c1);

                        lastRow.addElement(testTable);
                        
                        //lastRow = new PdfPCell(new Paragraph("  " + item_name.get(loop).toString()+"\n  Note: "+item_desc.get(loop).toString(), small));
                    }
                    lastRow.setBorderWidth(1f);
                    lastRow.disableBorderSide(PdfPCell.TOP);
                    lineTable.addCell(lastRow);

//                    lastRow = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat3d.format(Double.parseDouble(item_rate.get(loop).toString())), small));
//                    lastRow.setBorderWidth(1f);
...
//                    rateTotal += Double.parseDouble(item_rate.get(loop).toString());
                    ...
                    PdfPCell cellrateV;
                    cellrateV = new PdfPCell(unitRateCom);
                    cellrateV.setBorderWidth(1f);
                    cellrateV.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cellrateV.disableBorderSide(PdfPCell.TOP);
                    lineTable.addCell(cellrateV);


                    
//                    lastRow = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat.format(Double.parseDouble(item_value.get(loop).toString())), small));
//                    lastRow.setBorderWidth(1f);
//                    lastRow.setHorizontalAlignment(Element.ALIGN_RIGHT);
//                    lastRow.disableBorderSide(PdfPCell.TOP);
//                    lineTable.addCell(lastRow);
                    ...
                    if(lineItemDispChk)
                    {
                    Row = new PdfPCell(new Paragraph(line_item_no.get(loop).toString(), small));
                    ...
                    //lineTable.addCell(new Paragraph(currencySymbol+numberFormat.format(Double.parseDouble(item_value.get(loop).toString())), small));
                    Row.setHorizontalAlignment(Element.ALIGN_CENTER);
                    Row.disableBorderSide(PdfPCell.TOP);
                    Row.disableBorderSide(PdfPCell.BOTTOM);
                    Row.setBorderWidth(1f);
                    ...
                    if(item_desc.get(loop).toString().equals("")) {
                        ...
                        lineTable.addCell(Row);
                        //lineTable.addCell(new Paragraph("  " + item_name.get(loop).toString()+"\n  Note: "+item_desc.get(loop).toString(), small));
                    }
                    
//                    Row = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat3d.format(Double.parseDouble(item_rate.get(loop).toString())), small));
//                    //lineTable.addCell(new Paragraph(currencySymbol+numberFormat.format(Double.parseDouble(item_rate.get(loop).toString())), small));
...
//                    lineTable.addCell(Row);
                    ...
                    cellCurrncyV = new PdfPCell(currncyUnitCom);
                    cellCurrncyV.setBorderWidth(1f);
                    cellCurrncyV.setHorizontalAlignment(Element.ALIGN_RIGHT);
                    cellCurrncyV.disableBorderSide(PdfPCell.TOP);
...
//                    Row = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat.format(Double.parseDouble(item_value.get(loop).toString())), small));
//                    //lineTable.addCell(new Paragraph(currencySymbol+numberFormat.format(Double.parseDouble(item_value.get(loop).toString())), small));
//                    Row.setHorizontalAlignment(Element.ALIGN_RIGHT);
//                    Row.disableBorderSide(PdfPCell.TOP);
//                    Row.disableBorderSide(PdfPCell.BOTTOM);
...
//                    lineTable.addCell(Row);
                    ...
                    cellrateV = new PdfPCell(unitRateCom);
                    cellrateV.setBorderWidth(1f);
                    cellrateV.setHorizontalAlignment(Element.ALIGN_RIGHT);
                    cellrateV.disableBorderSide(PdfPCell.TOP);
                    cellrateV.disableBorderSide(PdfPCell.BOTTOM);
                    lineTable.addCell(cellrateV);
            ...
            /*
            if(rateTotal > 0) {
                lastRow = new PdfPCell(new Paragraph("  $ "+rateTotal.toString(), subFont));
                lastRow.setBorderWidth(1f);
                lastRow.disableBorderSide(PdfPCell.TOP);
                lineTable.addCell(lastRow);
            }
            */
            ...
            if(estTotal > 0) {
                lastRow = new PdfPCell(new Paragraph(currencySymbol+" "+numberFormat.format(estTotal), subFont));
                lastRow.setBorderWidth(1f);
                lastRow.setHorizontalAlignment(Element.ALIGN_RIGHT);
                lastRow.disableBorderSide(PdfPCell.TOP);
                lineTable.addCell(lastRow);
