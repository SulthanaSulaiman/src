...
public class ChennaiProductionRpt1DES

{
 ...
 public ChennaiProductionRpt1DES() /* this method continously checks the server time everyday*/
     {
   		/*getServerTime();
   		timer1 = new Timer();
...
public void ChennaiProductionReport(){
	...
	con = DriverManager.getConnection("jdbc:mysql://10.1.1.9:3306/pathfinderv2?jdbcCompliantTruncation=false&zeroDateTimeBehavior=convertToNull", "pfv2", "pfv2");
			...
			ResultSet rsGetDisplayDate = staccess.executeQuery("select DATE_FORMAT(CURRENT_TIMESTAMP(),'%d %M %Y'),DATE_FORMAT(CURRENT_TIMESTAMP(),'%r'),DATE_FORMAT(CURRENT_TIMESTAMP(),'%M'),DATE_FORMAT(CURRENT_TIMESTAMP(),'%Y') ");
			...
			int htmaccessupdate = staccess.executeUpdate("update project_update set update_time='"+accessconcatTime+"' where projgroup_id='7' ");

			ResultSet rsGet_TZfixed = staccess.executeQuery("select timezone_fixed from projectgroupname where projgroup_id='7' ");
			...
			while(rsGet_TZfixed.next())
				...
				{
					ResultSet rsGet_Tz = staccess.executeQuery(" select timezone_code,timezone_id,reference_timezone,time_difference_param,diff_hours,diff_minutes from timezone");
					...
					generateTzReport(timezone_code,timezone_id,timezone_ref,timezone_diffparam,timezone_diffhrs,timezone_diffmts);
}
...
public List generateTzReport(List tz_code,List tz_id,List tz_ref,List tz_diffparam,List tz_diffhr,List tz_diffmt)
{
...
//D:\\Sun\\AppServer\\domains\\domain1\\applications\\j2ee-modules\\pathfinder\\GPXL Data
		String XLFolder = "X:\\Pathfinder\\CPR\\V2_Reports\\Design Status-"+db_timezone_code+"\\";
		String HTMFolder = "D:\\Sun\\AppServer\\domains\\domain1\\applications\\j2ee-modules\\pathfinder\\GPHTM Data\\Design Status-"+db_timezone_code+"\\";
		


		/*String XLFolder = "C:\\Sun\\AppServer\\domains\\domain1\\applications\\j2ee-modules\\pathfinder\\GPXL Data\\Design Status-"+db_timezone_code+"\\";
		String HTMFolder = "C:\\Sun\\AppServer\\domains\\domain1\\applications\\j2ee-modules\\pathfinder\\GPHTM Data\\Design Status-"+db_timezone_code+"\\";*/
...
/******** Directories and filename creation ***/
	...
	if(!attachmentsDir.exists())
	 {		
            createFolder = attachmentsDir.mkdirs();		
         }

	System.out.println("createFolder:"+createFolder);

				String xlfilename = "Design Status-"+db_timezone_code+".xls";
	...
	if(!attachmentsDir2.exists())
	 {
             attachmentsDir2.mkdirs();		
				...
				String htmfilename = "Design Status-"+db_timezone_code+".htm";
...
/*** end of first style **/
			...
			headerCell1.setCellValue("Design Status");
...
dueDay.clear();
			...
			String current_date="";
			//ResultSet rsGetDueDate = statement.executeQuery("select distinct(date_format(a.due_date,'%d-%b-%y')),dayname(a.due_date),date_format(current_timestamp(),'%d-%b-%y') from chapter a,projectgroup b where (a.ship_date is null and a.proj_id=b.proj_id and b.projgroup_id='6') or (a.ship_date='0000-00-00 00:00:00' and a.proj_id=b.proj_id and b.projgroup_id='6') and a.chapter_status=1 and a.chapter_deleted=0 order by a.due_date");
			
ResultSet rsGetDueDate = statement.executeQuery("select distinct(date_format(a.due_date,'%d-%b-%y')),dayname(a.due_date),date_format(current_timestamp(),'%d-%b-%y') from chapter a,projects b where (a.ship_date is null and a.due_date is not null and a.proj_id=b.proj_id and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and a.stage  in ('DES','DESUPD','DESCXS','DESEVA')) or (a.ship_date='0000-00-00 00:00:00' and a.due_date is not null and a.proj_id=b.proj_id and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and a.stage  in ('DES','DESUPD','DESCXS','DESEVA')) order by a.due_date");
...
//Get the projects which does not have due date
		       ...
		       dueTZ_estimatedDateTime.clear();
                    ...
                    // ResultSet rsNoDue=statement.executeQuery("select distinct(b.proj_id),b.proj_name,c.stage,a.chapter_no,a.chapter_id,a.remark,date_format(a.estimated_time,'%d-%b-%y'),date_format(a.receipt_date,'%d-%b-%y'),date_format(a.estimated_time,'%h:%i %p'),a.mss_count,date_format(a.estimated_time,'%d-%m-%Y %h:%i %p') from chapter a, projects b, project_stage c,client g,projectgroup f where (a.ship_date is null or a.ship_date='0000-00-00 00:00:00') and c.stage_code=a.stage and a.proj_id=b.proj_id and a.due_date_received='0' and a.chapter_status=1 and a.chapter_deleted=0  and a.stage   in ('DES','DESUPD','DESCXS','DESEVA') order by a.estimated_time");

//this part is different from he Chennai Production Report because its not working with single row retrieval as Ordering has been used at the end of query

String Arc_NoDue="";

int Arc_NoDue_Count=0;

System.out.println("htmfilename:"+htmfilename);

Arc_NoDue="select distinct(b.proj_id),b.proj_name,c.stage,a.chapter_no,a.chapter_id,a.remark,date_format(a.estimated_time,'%d-%b-%y'),date_format(a.receipt_date,'%d-%b-%y'),date_format(a.estimated_time,'%h:%i %p'),a.mss_count,date_format(a.estimated_time,'%d-%m-%Y %h:%i %p') from chapter a, projects b, project_stage c where (a.ship_date is null or a.ship_date='0000-00-00 00:00:00') and a.due_date is not null and c.stage_code=a.stage and a.proj_id=b.proj_id and a.due_date_received='0' and a.stage   in ('DES','DESUPD','DESCXS','DESEVA') and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2'";


 ResultSet rsNoDue_chk=statement.executeQuery(Arc_NoDue);
	while(rsNoDue_chk.next())
		{
...
//System.out.println("Arc_NoDue_Count:"+Arc_NoDue_Count);

	if(Arc_NoDue_Count==1)
		{
System.out.println("htmfilename:"+htmfilename);
			Arc_NoDue="select distinct(b.proj_id),b.proj_name,c.stage,a.chapter_no,a.chapter_id,a.remark,date_format(a.estimated_time,'%d-%b-%y'),date_format(a.receipt_date,'%d-%b-%y'),date_format(a.estimated_time,'%h:%i %p'),a.mss_count,date_format(a.estimated_time,'%d-%m-%Y %h:%i %p') from chapter a, projects b, project_stage c where (a.ship_date is null or a.ship_date='0000-00-00 00:00:00') and a.due_date is not null and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and c.stage_code=a.stage and a.proj_id=b.proj_id and a.due_date_received='0' and a.stage   in ('DES','DESUPD','DESCXS','DESEVA') ";

		}
	else if(Arc_NoDue_Count>1)
		{
			Arc_NoDue="select distinct(b.proj_id),b.proj_name,c.stage,a.chapter_no,a.chapter_id,a.remark,date_format(a.estimated_time,'%d-%b-%y'),date_format(a.receipt_date,'%d-%b-%y'),date_format(a.estimated_time,'%h:%i %p'),a.mss_count,date_format(a.estimated_time,'%d-%m-%Y %h:%i %p') from chapter a, projects b, project_stage c where (a.ship_date is null or a.ship_date='0000-00-00 00:00:00') and a.due_date is not null and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and c.stage_code=a.stage and a.proj_id=b.proj_id and a.due_date_received='0' and a.stage   in ('DES','DESUPD','DESCXS','DESEVA') order by a.estimated_time";
		
		}


System.out.println("Arc_NoDue:"+Arc_NoDue);


 ResultSet rsNoDue=statement.executeQuery(Arc_NoDue);
	...
	while(rsNoDue.next())
			...
			System.out.println("Proj ID:"+rsNoDue.getString(1));
...
System.out.println("dueProjId-chk point:"+dueProjId);
...
//System.out.println("loop_DueDate:"+dueDate_List);
		...
		//System.out.println("dataStart_Row:"+i);
					...
					//String dispDataQuery1 = "select b.proj_name, date_format(a.due_date,'%d-%b-%y'), c.stage,b.proj_id,a.stage,date(a.due_date),a.stage from chapter a, projects b, project_stage c,client g where (a.ship_date is null and c.stage_code=a.stage and a.proj_id=b.proj_id and date_format(a.due_date,'%d-%b-%y')='"+loop_DueDate+"' )or(a.ship_date = '0000-00-00 00:00:00' and c.stage_code=a.stage and a.proj_id=b.proj_id  and date_format(a.due_date,'%d-%b-%y')='"+loop_DueDate+"' ) and a.chapter_status=1 and a.chapter_deleted=0  group by a.stage,a.proj_id,a.due_date,a.estimated_time order by a.due_date ";

					String dispDataQuery1 = "select b.proj_name, date_format(a.due_date,'%d-%b-%y'), c.stage,b.proj_id,a.stage,date(a.due_date),a.stage,a.due_date_received,date_format(a.receipt_date,'%d-%b-%y'),a.chapter_process from chapter a, projects b, project_stage c where (a.chapter_no not like '%To Delete%'  and a.ship_date is null and c.stage_code=a.stage and a.proj_id=b.proj_id and date_format(a.due_date,'%d-%b-%y')='"+loop_DueDate+"' and a.due_date_received='1' and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and a.stage   in ('DES','DESUPD','DESCXS','DESEVA'))or(a.chapter_no not like '%To Delete%' and a.ship_date = '0000-00-00 00:00:00' and c.stage_code=a.stage and a.proj_id=b.proj_id  and date_format(a.due_date,'%d-%b-%y')='"+loop_DueDate+"' and a.due_date_received='1' and a.chapter_status=1 and a.chapter_deleted=0 and b.project_status <> '2' and a.stage   in ('DES','DESUPD','DESCXS','DESEVA')) group by a.stage,a.proj_id,a.due_date order by a.due_date ";
...
//					System.out.println("proj_Name:"+proj_Name);
			...
			//	System.out.println("dispProject:"+dispProject);
			 ...
			 if(dueProjId.size()>0)
				...
				System.out.println("tableStart_Row-withno due:"+tableStart_Row);
			...
			s+="</table> </td></tr></div></div></div>";
					...
					System.out.println("End Of Chennai Production Report");
